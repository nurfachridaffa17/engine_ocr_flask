...
def extract_ktp_data(text_response,debug_mode=False):
    ...
    if (len(attributes['identity_number']) != 16):
            attributes['state'] = "invalid NIK"
    ...
    if(raw_result['ttl'] != None):
        ...
        if(len(ttls)>=2):
            ...
            attributes['birth_date'] = extract_date(ttls[1]).date()
    ...
    if(attributes['birth_place'] != None):
        attributes['birth_place'] = ''.join([i for i in attributes['birth_place'] if not i.isdigit()]).replace('-','').replace('.','').replace('/','').replace('Tgl', '').strip()
    ...
    if(attributes['marital_status'] != None):
        ...
        elif(levenshtein('cerai hidup',attributes['marital_status'].lower()) <= 2):
            attributes['marital_status'] = 'Cerai Hidup'
        elif(levenshtein('cerai mati',attributes['marital_status'].lower()) <= 2):
            attributes['marital_status'] = 'Cerai Mati'
    ...
    attributes['religion'] = re.sub(r'[^a-zA-Z]','', raw_result['agama'].split(' ', 1)[0])
...
def process_extract_entities(ocr_path):
        ...
        # except Exception as e:
            # print(ocr_path+' cannot be loaded'+ str(e))
        ...
        output_name = cfg.output_loc+'data_'+ocr_name+'.json'
        # ktp_extract.to_json(output_name,orient='records',index=True, indent=4)
