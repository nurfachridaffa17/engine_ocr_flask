...
from .models import db, t_attachment, M_Member, t_log_attachment, m_status
import boto3

@app.route('/dowload/ktp', methods=['POST', 'GET'])
def dowload_ktp():
    img_var ='data_ktp_muslimat_nu/{}'.format(request.form.get('variabel_image'))
    ...
    if img_var is not exists:
        ...
        client.download_file(s3_bucket, 'image/ktp/{}'.format(nama_file), 'data_ktp_muslimat_nu/%s'%(nama_file))
...
def image_ktp():
    ...
    nama_file = request.form.get('variabel_image')
    img_var ='data_ktp_muslimat_nu/{}'.format(nama_file)
    s3_bucket = app.config['S3_BUCKET']
    s3_dir = app.config['S3_DIR']
    session = boto3.session.Session()
    client = session.client(
    ...
    hasil_crop = cv2.imwrite('./resource/%s'%(nama_file), img_transform)
    if not hasil_crop:
        return jsonify(message='No File Uploaded'), 400
    else:
        with open('./resource/%s'%(nama_file), 'rb') as src:
            client.put_object(
                ACL='public-read',
                Bucket=s3_bucket,
...
def get_ktp():
    if request.method == 'PUT':
        ...
        img_path ='data_ktp_muslimat_nu/{}'.format(request.form.get('variabel'))
        ...
        if check_member.nik is None:
            ...
            if id_member is None:
                    ...
                    os.remove('data_ktp_muslimat_nu/%s'%(request.form.get('variabel')))
                    os.remove('resource/%s'%(request.form.get('variabel')))
        else:
